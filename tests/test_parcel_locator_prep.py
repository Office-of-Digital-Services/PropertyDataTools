...
from .config import ALPINE_SMARTFABRIC, ALAMEDA_SMARTFABRIC, AMADOR_SMARTFABRIC

from unbox import build_locator

TEST_OUTPUTS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs")
...
def test_full_locator_build(smartfabric=INPUT_SMARTFABRIC_STATEWIDE, locator=os.path.join(OUTPUT_FOLDER, "test_statewide_locator_20250116.loc"),
...
def test_alpine_parcel_prep(parcels=os.path.join(ALPINE_SMARTFABRIC, "parcels"), assessments=os.path.join(ALPINE_SMARTFABRIC, "assessments"), output_folder=TEST_OUTPUTS):
    build_locator.prepare_locator_data(parcels, assessments, output_folder)
